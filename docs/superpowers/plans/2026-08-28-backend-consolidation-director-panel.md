...
- [x] Commit: `git add -A && git commit -m "chore: remove root Prisma layer, root app is now backend/-only"` — commit `d5cc994` (code-side only; the DB-drop step above has no code artifact to commit — it was completed separately, see line above)
