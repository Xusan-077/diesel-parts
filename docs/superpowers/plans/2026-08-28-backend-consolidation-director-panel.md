...
- [ ] Update `package.json`, run `npm install` to regenerate the lockfile without the removed deps
- [ ] `npx tsc --noEmit && npm run lint && npm run build && npm test`
- [ ] **Stop and confirm with the user before this step** (CLAUDE.md's stop-condition on destructive/production-adjacent actions — dropping a database, even a local dev one with real work in it, is exactly the kind of action that rule exists for): take a final `pg_dump` of `diesel_parts_web_dev` alongside the one already taken in Part 2, Task 11, then `DROP DATABASE diesel_parts_web_dev` locally once the user has confirmed the cutover is fully verified and they don't want to keep the old database around as a live fallback for a few more days
- [ ] Commit: `git add -A && git commit -m "chore: remove root Prisma layer, root app is now backend/-only"`
...
- [ ] After this plan's Part 5 is complete, re-open `docs/superpowers/plans/2026-08-28-checkout-order-types-expansion.md` and strike its Global Constraints bullet about the two-database split (the one quoted above) — add a note pointing at this plan instead of deleting the history outright, e.g. `~~Extending the director panel...~~ Resolved by docs/superpowers/plans/2026-08-28-backend-consolidation-director-panel.md — the director panel now reads backend/ directly.`
- [ ] `app/director/**` gains visibility into self-checkout orders (Payme, Click, Paynet, Cash, pending quotes) for free once Task 18 (Orders domain rewire) lands, since `order-repository.ts` will be calling the same `backend/` `seller/orders` endpoint the seller panel already uses, and a `DIRECTOR`-role JWT already gets the unscoped view via `orderReadScope`. No new director-specific order UI work is implied by this — the existing `app/director/(panel)/**` order-related pages (if any list orders today) start seeing self-checkout orders alongside CRM ones automatically. Verify this specifically as part of Task 27's smoke test: place a self-checkout order (any method), confirm it's visible from the director panel's order view, not just the seller panel's.
- [ ] If the director panel today has **no** dedicated "orders" list page at all (check `app/director/(panel)/**` — this session did not find one distinct from `analytics`/`customers`/`discounts`/`products`/`reviews`/`users`/`warehouse`/`audit`/`categories`), building one is genuinely new scope, not something this plan's rewire delivers "for free" — note this explicitly in the closing report rather than silently claiming it's covered, and leave it as a follow-up the user can decide to spec separately.
