...
- [ ] Boot both (`npm run dev` in `backend/`, `npm run dev` at root) and smoke-test end-to-end once more: storefront browsing/review/inquiry submission; director login, product CRUD + CSV import/export + image upload, category tree edit, discount request + approval, inquiry board claim, audit log view, staff user management, warehouse/stock view; seller panel (`/login`, `/seller/**`) untouched and still working (confirms Part 3/4 didn't regress it); legacy `/admin/seller` still working — **not run this session** (no browser session available); the automated checks above cover the code paths, but this manual end-to-end pass is still open
