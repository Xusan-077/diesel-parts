...
export async function generateMetadata({
  ...
  const { data: brands } = await safeRead("brand metadata", listBrands, [] as Brand[]);
...
export default async function BrandDetailPage({
  ...
  /*
   * The brand row carries this page's title and its identity, so an unreadable
   * one leaves nothing to degrade — the notice becomes the page. Kept apart
   * from the missing-row case below, which is a real 404.
   */
  const lookup = await safeRead("brand", listBrands, [] as Brand[]);
  if (!lookup.ok) {
    return (
  ...
  const brand = lookup.data.find((b) => b.slug === slug);
  if (!brand) {
    notFound();
  }
  ...
  const [page, categories] = await Promise.all([
    ...
    safeRead("brand page categories", listCategories, [] as Category[]),
  ]);
    ...
    <Container as="main" className="pb-24 pt-6">
      ...
      {page.data ? (
        <p className="mt-2 text-muted">
          {page.data.total} {dict.brands.productsFromBrand}
        </p>
      ) : null}
