 ...
 *
 * An audit line carrying every column makes a one-word note change look like a
 * rewrite of the row, and the trail stops being readable at exactly the moment
 * somebody needs to read it. Pure, so the comparison rules — including dates,
 * which are never equal by identity — are asserted in a test.
 */
