...
describe("diffFields", () => {
  it("returns null when nothing moved", () => {
    // A seller pressing save on an unchanged form is not an event worth a line
    // in the trail.
    expect(diffFields({ status: "NEW", notes: null }, { status: "NEW", notes: null })).toBeNull();
  });
  ...
  it("carries only the fields that changed", () => {
    ...
    expect(diff).toEqual({ before: { status: "NEW" }, after: { status: "WON" } });
  });
  ...
  it("compares dates by their instant, not by identity", () => {
    ...
    expect(diffFields({ followUpAt: new Date(when) }, { followUpAt: new Date(when) })).toBeNull();
  });
  ...
  it("reports a date that actually moved", () => {
    ...
    expect(diff?.after.followUpAt).toEqual(new Date("2026-09-08T00:00:00.000Z"));
  });

  it("does not treat a number and its string form as equal", () => {
    expect(diffFields({ percent: 5 } as Record<string, unknown>, { percent: "5" })).not.toBeNull();
  });
});
