...
describe('formatNationalDigits', () => {
  ...
  it('is idempotent', () => {
    expect(formatNationalDigits(formatNationalDigits('901234567'))).toBe(
      '90 123 45 67',
    );
  });
});
...
describe('phoneTail', () => {
  it('returns the last two national digits', () => {
    expect(phoneTail('+998 90 123-45-67')).toBe('67');
    expect(phoneTail('998901234567')).toBe('67');
  });

  it('returns an empty string when there are fewer than eight national digits', () => {
    expect(phoneTail('+998 90 12')).toBe('');
  });
});
